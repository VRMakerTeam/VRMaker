...
namespace VRXX.Action.Shared
{
    public static class PunchScale
    {
        ...
        public static void Execute(Dictionary<string, string> _params, ActionDelegate _onFinish)
        {
            ...
            {
                Log.Error("PunchScale", "need params targetGameObject");
                return;
            }
            string x = "";
            if (!_params.TryGetValue("x", out x))
            {
                Log.Error("PunchScale", "need params x");
                return;
            }
            ...
            {
                Log.Error("PunchScale", "need params y");
                return;
            }
            ...
            {
                Log.Error("PunchScale", "need params z");
                return;
            }
            string duration = "";
            if (!_params.TryGetValue("duration", out duration))
            {
                Log.Error("PunchScale", "need params time");
                return;
            }
            ...
            try
            {
                ...
                if (go != null)
                {
                    ...
                    Log.Error("PunchScale", "[{0}] GameObject is not exist...", targetGameObject);
            }
            catch (System.Exception e)
            {
                Log.Error("PunchScale", "Parse json has error: " + e.Message);
            }
        }


        private static void punchScale(GameObject _go, Vector3 _position, float _duration, ActionDelegate _onFinish)
        {
            Hashtable hash = new Hashtable();
            hash.Add("amount", _position);
            hash.Add("time", _duration);

            iTween.PunchScale(_go, hash);
